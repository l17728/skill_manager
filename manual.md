...
**版本 2.3 · 适用平台：Windows 10 / Windows 11**
...
点击顶部右侧的 **? 手册** 按钮，即可在独立窗口中打开本用户手册的完整渲染版本。手册窗口与主窗口并存，可同时操作。重复点击会聚焦已有手册窗口，不会重复弹出。
...
> **关于 Purpose 和 Provider**：这两个字段是**自定义分类标签**，你可以填任意英文字符串。用于将 Skill 分层归档，便于管理。例如：Purpose 填 `coding` 表示这是用于写代码的 Skill；Provider 填 `internal` 表示是你自己写的。点击这两个输入框时，会出现下拉提示列表，显示已有的 Purpose / Provider 值，可直接选择复用已有分类，无需重新手动输入。
...
**版本标识**：列表中每个 Skill 条目的右侧显示当前版本号（如 `v1`、`v2`），有助于区分同名 Skill 的不同迭代版本。
...
**空列表引导卡**：首次使用（尚未导入任何 Skill）时，列表区域显示引导卡，卡片中附有快速导入按钮，点击即可直接打开导入弹窗。
...
> **Purpose 和 Provider 输入提示**：点击这两个字段时，会出现下拉提示列表，显示已有值，可直接选择复用已有分类。
...
> **JSON 模板下载**：不确定格式？点击上传区域旁的 **↓ 下载 JSON 模板** 按钮，即可下载含示例内容的模板文件，填入自己的用例后上传即可。
...
**空列表引导卡**：首次使用（尚未导入任何基线）时，列表区域显示引导卡，卡片中附有快速导入按钮，点击即可直接打开导入弹窗。
...
**点击左侧列表中的项目名称**，右侧面板展开详情。详情面板顶部有**工作流进度指示器**，显示 4 个步骤（① Test → ② Analysis → ③ Recompose → ④ Iteration），已完成的步骤高亮，帮助你随时了解项目所处阶段。
...
- **克隆**：选中项目 → 详情面板右上角 **Clone** 按钮，系统复制完整项目配置（含关联的 Skill 和基线），新项目名称自动加"-副本"后缀，状态重置为"未开始"
- **删除**：选中项目 → 详情面板右上角 **Delete** 按钮 → 确认

> ⚠️ 删除项目**不可恢复**，项目下所有测试结果和分析报告将一同删除。

**空列表引导卡**：首次使用项目页面（尚未创建任何项目）时，列表区域显示引导卡，卡片中附有"创建项目"快速按钮，点击即可直接打开创建弹窗。
...
**用例明细**：在展开的行中，点击 **用例 ▾** 按钮可进一步查看每条测试用例的单独得分和 AI 实际输出，便于定位哪道题答得好、哪道题有改进空间。
...
>
> ⚠️ **前提**：该项目的测试状态为"已完成"。若测试尚未完成，Analysis 标签页会显示提示横幅（"请先完成测试，才能运行分析"），点击横幅中的跳转链接可直接切换到 Test 标签页。
...
>
> ⚠️ **前提**：该项目的差异分析已完成。若分析尚未运行，Recompose 标签页会显示提示横幅（"请先运行差异分析，才能执行重组"），点击横幅中的跳转链接可直接切换到 Analysis 标签页。
...
重组完成后，保存表单会**自动预填**最优 Skill 的 Name、Purpose 和 Provider，无需手动输入：

1. 确认或修改预填的 Skill 名称（默认：`<原名称>-recomposed`）
2. 确认 **Purpose** 和 **Provider** 字段（已与来源 Skill 保持一致）
3. 点击 **Save as New Skill**
...
- 每轮的 Skill 版本和得分
- 分数变化曲线（是否持续提升）
- 各轮使用的优化策略（界面以中文显示：**全面强化** / **聚焦维度** / **片段探索** / **交叉融合** / **随机子集**；鼠标悬停在策略名上可查看对应英文名称）
- **最优版本推荐**（分数最高的那一轮）
...
如果你想用相同的 Skill / 基线组合再跑一次独立实验，或把某个项目存档一份，可以使用克隆功能（无需手动重建项目）：

1. 在 Projects 页面选中目标项目
2. 点击详情面板右上角 **Clone** 按钮
3. 系统自动创建副本，新项目名称为"`原名称-副本`"

克隆项目继承原项目全部 Skill 和基线配置，但**不复制测试结果**（状态重置为"未开始"），适合以相同配置重新执行测试。
...
### ❓ 问题 7：Analysis 或 Recompose 标签页显示"前置条件未满足"横幅
...
- Analysis 标签显示"请先完成测试，才能运行分析" → 该项目测试尚未完成，点击横幅中的链接跳转到 Test 标签完成测试
- Recompose 标签显示"请先运行差异分析，才能执行重组" → 分析尚未运行，点击横幅中的链接跳转到 Analysis 标签运行分析
